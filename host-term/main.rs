...
fn main() {
    let (dev_bt_addr, baud_rate, read_timeout_ms, hex_mode, clear_on_disc) = {
        ...
        (dev_bt_addr.unwrap(), baud_rate, read_timeout_ms, hex_mode, clear_on_disc)
    };
    ...
    ble_ser.lock().unwrap().on_event(move |evt| {
        let ble_ser = if let Some(s) = ble_ser_weak.upgrade() { s } else { return };
        match evt {
            BleSerialEvent::Connect => {
                ...
                if let Some(baud) = baud_rate {
                    match ble_ser.lock().unwrap().set_baud_rate(baud) {
                        ...
                        Err(opt_b) =>
                            println!("BleSerial: Baudrate not set. expected: {} current: {:?}", baud, opt_b),
                    }
                } else {
                    println!("BleSerial: Baudrate {}",
            ...
            BleSerialEvent::WriteFailed(data) => {
                println!("BleSerial Event: WriteFailed {}", &bytes_to_spaced_hex(&data));
            },
        }
    }).unwrap();
    ...
    loop {
        ...
        if let Err(_) = io::stdin().read_line(&mut cmd_line) {
            return;
        }
...
fn bytes_to_spaced_hex(bytes: &[u8]) -> String {
    let s_tmp = bytes.encode_hex::<Vec<char>>();
    let mut s = String::new();
    for h in s_tmp.chunks(2) {
        s.push(h[0]); s.push(h[1]); s.push(' ');
    }
    return s;
}
